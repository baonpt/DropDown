...
public final class DropDown: UIView {
    ...
    public var arrowIndicationX: CGFloat? {
        didSet {
            if let arrowIndicationX = arrowIndicationX {
                ...
                arrowIndication.tintColor = settings.interface.backgroundColor
    ...
    public var settings: DropDownSetting = .init() {
        willSet {
            /// Row height
            tableView.rowHeight = settings.interface.rowHeight
            /// Table view background color
            tableView.backgroundColor = settings.interface.backgroundColor
            if arrowIndicationX != nil {
                arrowIndication.tintColor = settings.interface.backgroundColor
            }
            /// Corner
            tableViewContainer.layer.cornerRadius = settings.interface.cornerRadius
            tableView.layer.cornerRadius = settings.interface.cornerRadius
            /// Separator
            tableView.separatorColor = settings.interface.separatorColor
            /// Shadow
            tableViewContainer.layer.shadowColor = settings.interface.shadow.color.cgColor
            tableViewContainer.layer.shadowOffset = settings.interface.shadow.offset
            tableViewContainer.layer.shadowOpacity = settings.interface.shadow.opacity
            tableViewContainer.layer.shadowRadius = settings.interface.shadow.radius
            /// Animation
            tableView.transform = settings.animation.downScaleTransform
        }
        didSet {
            reloadAllComponents()
            setNeedsUpdateConstraints()
    ...
    public override var backgroundColor: UIColor? {
        get { return  settings.interface.backgroundColor }
        set { settings.interface.backgroundColor = newValue! }
    }
    ...
    public var cellNib = UINib(nibName: "DropDownCell", bundle: bundle) {
        didSet {
            tableView.register(
                cellNib,
                forCellReuseIdentifier: DropDownSetting.cellIdentifier
            )
            templateCell = nil
            reloadAllComponents()
        }
...
extension DropDown {
    ...
    public override func layoutSubviews() {
        ...
        let shadowPath = UIBezierPath(
            roundedRect: tableViewContainer.bounds,
            cornerRadius: settings.interface.cornerRadius
        )
    ...
    barButtonItemCondition: if let anchorView = anchorView as? UIBarButtonItem {
        ...
        let menuWidth = settings.interface.menuWidth ?? fittingWidth()
        let anchorViewWidth = anchorView.plainView.frame.width
        let x = -(menuWidth - anchorViewWidth)
    ...
    fileprivate func computeLayoutBottomDisplay(window: UIWindow) -> ComputeLayoutTuple {
        ...
        let menuWidth = settings.interface.menuWidth ?? (anchorView?.plainView.bounds.width ?? fittingWidth()) - bottomOffset.x
        
        let anchorViewX = anchorView?.plainView.windowFrame?.minX ?? window.frame.midX - (menuWidth / 2)
        ...
        let windowMaxY = window.bounds.maxY - settings.interface.heightPadding - offsetFromWindowBottom
        
        let keyboardListener = KeyboardListener.sharedInstance
        let keyboardMinY = keyboardListener.keyboardFrame.minY - settings.interface.heightPadding
        ...
        return (x, y, menuWidth, offscreenHeight)
    }
    ...
    fileprivate func computeLayoutForTopDisplay(window: UIWindow) -> ComputeLayoutTuple {
        ...
        let windowY = window.bounds.minY + settings.interface.heightPadding
        ...
        let menuWidth = settings.interface.menuWidth ?? (anchorView?.plainView.bounds.width ?? fittingWidth()) - topOffset.x
    ...
    fileprivate func fittingWidth() -> CGFloat {
        ...
        for index in 0..<dataSource.count {
            ...
            templateCell.bounds.size.height = settings.interface.rowHeight
            let menuWidth = templateCell.systemLayoutSizeFitting(UIView.layoutFittingCompressedSize).width
    ...
    fileprivate func constraintWidthToFittingSizeIfNecessary(layout: inout ComputeLayoutTuple) {
        guard settings.interface.menuWidth == nil else { return }
...
extension DropDown {
    ...
    public func show(onTopOf window: UIWindow? = nil, beforeTransform transform: CGAffineTransform? = nil, anchorPoint: CGPoint? = nil) -> (canBeDisplayed: Bool, offscreenHeight: CGFloat?) {
        ...
        if transform != nil {
            ...
            tableViewContainer.transform = settings.animation.downScaleTransform
        }
        
        layoutIfNeeded()
        
        UIView.animate(
            withDuration: settings.animation.duration,
            delay: 0,
            options: settings.animation.entranceOptions,
    ...
    public func hide() {
        ...
        UIView.animate(
            withDuration: settings.animation.duration,
            delay: 0,
            options: settings.animation.exitOptions,
            animations: { [weak self] in
...
extension DropDown: UITableViewDataSource, UITableViewDelegate {
    ...
    fileprivate func configureCell(_ cell: DropDownCell, at index: Int) {
        ...
        cell.optionLabel.textColor = settings.interface.textColor
        cell.optionLabel.font = settings.interface.textFont
        cell.selectedBackgroundColor = settings.interface.selectionBackgroundColor
        cell.highlightTextColor = settings.interface.selectedTextColor
        cell.normalTextColor =  settings.interface.textColor
